...
module Command
  ...
  Examples:
    ...
    narouq setting convert.no-open=true   # 値を設定する
    narouq setting convert.no-epub=       # 右辺に何も書かないとその設定を削除出来る
    narouq setting device                 # 変数名だけ書くと現在の値を確認出来る

    narouq s convert.copy-to=C:/dropbox/mobi
    # パスにスペースが含まれる場合はダブルクウォーテーションで囲う
    narouq s convert.copy-to="C:\\Documents and Settings\\user\\epub"
